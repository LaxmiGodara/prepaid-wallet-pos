...
async function generateUniqueCardNumber(): Promise<string> {
  ...
  throw new AppError("Could not generate a unique card number. Please try again.", 500);
}
  ...
  try {
    ...
    if (existingCard) {
      await Card.findOneAndUpdate(
        ...
        { status: CARD_STATUS.REPLACED, updatedBy: actorOid },
        { session }
      );
    }
  ...
  [
    {
      ...
      createdBy: actorOid,
      updatedBy: null,
      // isDeleted and deletedAt are NOT on the Card model - removed
    },
  ],
  { session }
);
