...
import { lookupMemberByCard } from "@/lib/services/billing.service";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await lookupMemberByCard(cardNumber);
    return NextResponse.json(buildSuccessResponse("Member found.", result), { status: 200 });
