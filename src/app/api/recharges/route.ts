...
import { createRecharge, listRecharges } from "@/lib/services/recharge.service";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { rechargeList, total } = await listRecharges({ page, limit, search });

    return NextResponse.json(
      buildSuccessResponse("Recharges fetched successfully.", rechargeList, {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await createRecharge(body, actor.staffId);

    return NextResponse.json(
      buildSuccessResponse("Wallet recharged successfully.", result),
      { status: 201 }
    );
  } catch (error) {
    return handleApiError(error);
