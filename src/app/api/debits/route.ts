...
import { createDebit, listDebits } from "@/lib/services/debit.service";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { debitList, total } = await listDebits({ page, limit, search });

    return NextResponse.json(
      buildSuccessResponse("Debits fetched successfully.", debitList, {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await createDebit(body, actor.staffId);

    return NextResponse.json(
      buildSuccessResponse("Manual debit processed successfully.", result),
      { status: 201 }
    );
  } catch (error) {
    return handleApiError(error);
