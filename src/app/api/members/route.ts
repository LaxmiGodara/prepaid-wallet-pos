...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // createMember runs the atomic transaction internally
    const newMember = await createMember(body, actor.staffId);

    return NextResponse.json(
      buildSuccessResponse(
        "Member enrolled successfully with a new wallet.",
        newMember
      ),
      { status: 201 }
    );
