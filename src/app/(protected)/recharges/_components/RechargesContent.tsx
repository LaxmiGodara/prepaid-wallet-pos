...
import { PAGINATION, PAYMENT_MODES, RECORD_STATUS } from "@/lib/constants";
...
const PAYMENT_MODE_OPTIONS = Object.values(PAYMENT_MODES);
...
export default function RechargesContent() {
  const [list, setList] = useState<RechargeRecord[]>([]);
  ...
  const [paymentMode, setPaymentMode] = useState<string>(PAYMENT_MODES.CASH);
  ...
  const fetchList = useCallback(async (page: number, search: string) => {
    ...
    try {
      ...
      const auth = getAuthorizationHeader();
      const res = await fetch(`/api/recharges?${p}`, {
        headers: auth ? { Authorization: auth } : {},
      });
      const r = await res.json();
      if (!r.success) {
        setListError(r.message ?? "Failed to load recharges.");
        return;
      }
      setList(r.data as RechargeRecord[]);
    ...
    try {
      ...
      if (!r.success) {
        if (r.errors?.length > 0) {
          ...
          setCreateErr(r.message ?? "Recharge failed.");
        }
        return;
      }
      setShowCreate(false);
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      <PageHeader
        ...
        subtitle={
          meta.total > 0
            ? `${meta.total} recharge${meta.total !== 1 ? "s" : ""}`
            : "Add credit to member wallets"
        }
        actions={
          !showCreate ? (
            <Button
      ...
      {/* Create Form */}
      {showCreate && (
        <SectionCard title="New Recharge">
          <form
                ...
                <div className="flex items-center justify-between rounded-xl border border-[var(--color-accent-soft-line)] bg-[var(--color-accent-soft)]/40 px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Current balance:{" "}
                      <strong>
                        {formatCurrency(selectedMember.walletBalance)}
                      </strong>
                    </p>
                  </div>
                  <button
                      ...
                      {memberResults.map((m) => (
                        ...
                        >
                          ...
                          <p className="text-xs text-slate-400">
                            {m.mobileNumber ?? "No mobile"} · Balance:{" "}
            ...
            <div className="col-span-2 flex flex-col gap-3">
              ...
              <div className="flex gap-3">
                <Button type="submit" variant="primary" isLoading={isCreating}>
                  {isCreating ? "Processing..." : "Process Recharge"}
                </Button>
                <Button
                  type="button"
                  variant="secondary"
                ...
                {list.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-xs text-slate-500">
                      {formatCurrency(r.walletBalanceBefore)} →{" "}
                      <strong className="text-slate-700">
                        {formatCurrency(r.walletBalanceAfter)}
                      </strong>
                    </td>
                    <td className="px-6 py-4 text-xs text-slate-500">
                      {r.referenceNumber ?? "—"}
